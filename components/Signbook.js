...
function GuestbookEntry({ entry, user }) {
  ...
  return (
    <div className="flex flex-col space-y-2">
      <div className="prose dark:prose-dark w-full text-gray-800">
        {entry.body}
      </div>
      <div className="flex items-center space-x-3">
        <p className="text-sm text-gray-800">{entry.created_by}</p>
        <span className=" text-gray-200 dark:text-gray-800">/</span>
        <p className="text-sm text-gray-600 dark:text-gray-600">
          {format(new Date(parseInt(entry.epoch)), "d MMM yyyy 'at' h:mm bb")}
        </p>
...
export default function Guestbook({ initialEntries }) {
  ...
  const leaveEntry = async (e) => {
    ...
    setForm({
      state: 'success',
      message: `Hooray! Thanks for your comment.`
    });
  };
    ...
    <>
      <div className="border border-blue-200 rounded p-6 my-4 w-full dark:border-gray-800 bg-blue-50 dark:bg-blue-opaque">
        ...
        <p className="my-1 text-gray-800 dark:text-gray-200">
          Share a message for visitors of my site.
        </p>
        {user?.name ? (
